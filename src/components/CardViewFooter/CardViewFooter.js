...
class CardViewFooter extends Component {
  addSkillsArr() {
    ...
    if (dataSkills.length === 0) {
      return defaultSkills.map((defSkill, index) => {
        return (
          <li key={index} className="skills__item skills__item--bg">
            {defSkill}
          </li>
        );
      });
    ...
    return (
      <div className="card-footer">
        <div className="skills">
          <ul className="skills__list">{this.addSkillsArr()}</ul>
        </div>
      </div>
    );
  }
